...
4. `actionlock_preview` derives an action hash from the evidence, tool policy, and canonical argument hash.
...
The model cannot declare its own capability class, invent evidence, substitute arguments after approval, or replay an approval after restart.
...
Tests cover canonical Technocore signatures, big-integer evidence, signed receipt tampering and expiry, exact approval binding, persistent replay protection, concurrent audit appends, blocked sensitive capabilities, downstream argument substitution, and end-to-end gateway execution.
